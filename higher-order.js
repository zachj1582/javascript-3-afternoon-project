...
//Code Here
let postTaxPrices = prices.map((per) => per * 1.07)// = prices.map(/* Provide Your Callback Here );
...
let orderTotals = orders.map((value) => value.price * value.tax + value.price)
...
let bobsTotal = purchases.filter(value => value.owner === 'Bob').map(value => value.price).reduce((acc,curr) => acc + curr)
